...
def training_data_one_by_one_label(dataset, labels, data_format="instruction"):
    """
    This function processes each entry in the given dataset and creates multiple prompts,
    each designed to extract a single label. Depending on the chosen `data_format`, 
    ...
    Args:
        ...
        data_format: The format to use when formatting the data. It should be one of "instruction" or "conversational".
    ...
    if data_format == "instruction":
        for example in dataset:
            ...
            for label in labels:
                message = Prompts.instruction_training([label], example["text"], grouped_entities[label])
        ...
        for example in dataset:
            ...
            grouped_entities = {label: [item for item in relevant_labels if item['label'] == label] for label in labels}
            for label in labels:
                message = Prompts.conversational_training([label], example["text"], grouped_entities[label])
                train_format_dataset.append({"messages": message})
    else:
        raise ValueError("data_format must be one of 'instruction' or 'conversational'")
    return Dataset.from_list(train_format_dataset)

def training_data_all_labels(dataset: List[dict], labels: List[str], data_format: str = "instruction") -> Dataset:
    """
    ...
    `data_format`, it constructs the prompt in either instruction or conversational style.
    ...
    if data_format == "instruction":
        for example in dataset:
            relevant_labels = [label for label in example["labels"] if label["label"] in labels]
            message = Prompts.instruction_training(labels, example["text"], relevant_labels)
            train_format_dataset.append(message)
    elif data_format == "conversational":
        for example in dataset:
            relevant_labels = [label for label in example["labels"] if label["label"] in labels]
            message = Prompts.conversational_training(labels, example["text"], relevant_labels)
            train_format_dataset.append({"messages": message})
    else:
        raise ValueError("data_format must be one of 'instruction' or 'conversational'")
    return Dataset.from_list(train_format_dataset)
...
def conversations_formatting_function(examples: dict, tokenizer: AutoTokenizer):
    ...
    """
    #print("Input IDS prepared for: CONVERSATIONAL PROMPT")
    if isinstance(examples, list):
        output_texts = []
        for example in examples:
            output_texts.append(tokenizer.apply_chat_template(example["messages"], tokenize=False, add_generation_prompt=False))
        return output_texts
    else:
...
def formatting(examples, tokenizer, data_format="instruction"):
    """
    ...
    Args:
        ...
        tokenizer: The tokenizer to use.
        data_format: The format to use. One of "instruction" or "conversational".
...
def training(model, tokenizer, dataset, save_model_path):
    ...
    formatting_prompts_func = partial(formatting, tokenizer=tokenizer)
...
def main(args):
    ...
    if training_type == "training_data_one_by_one_label":
        dataset = training_data_one_by_one_label(dataset, labels, data_format)
    elif training_type == "training_data_all_labels":
        dataset = training_data_all_labels(dataset, labels, data_format)
    print(dataset)
    print("Dataset prepared")

    for model_name in models:
        model, tokenizer = prepare_model_and_tokenizer(model_name, use_gpu)
        training(model, tokenizer, dataset, args.save_model_path)

if __name__ == "__main__":
    parser = ArgumentParser()
    parser.add_argument("--script_args", type=str, help="Path to the script arguments JSON file.")
    parser.add_argument("--dataset", type=str, default=0.95, help="Train dataset.")
    parser.add_argument("--save_model_path", type=str, default=0.95, help="Save model path.")
